...
db_dir = os.environ.get('BERKELEYDB_PATH', '/data/berkeleydb')
os.makedirs(db_dir, exist_ok=True)
db = bdb.DB()
db.open(os.path.join(db_dir, 'bench.db'), dbtype=bdb.DB_BTREE, flags=bdb.DB_CREATE)
db.get(b'_warmup')
    ...
    with open(result_path, 'w', newline='') as f:
        ...
        for i in range(phase1_ops):
            if i % 3 != 2:
                ...
                t0 = time.perf_counter()
                queue = json.loads(db.get(queue_key))
                queue.append(message)
                db.put(queue_key, json.dumps(queue).encode())
                elapsed = (time.perf_counter() - t0) * 1000
...
if __name__ == '__main__':
    run(CATEGORIES)
    # BDB buforuje zapisy w cache — jawny close flushuje je na dysk,
    # zanim kolejna kategoria (osobny docker exec) otworzy baze.
    db.close()
